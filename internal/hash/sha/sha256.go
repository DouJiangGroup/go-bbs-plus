...
func SHA256(message []bool) [8]uint32 {
	paddedMessage := padMessage(message, 512)

	h := h0

	for i := 0; i < len(paddedMessage); i += 512 {
		block := paddedMessage[i : i+512]

		// Prepare message schedule
		w := make([]uint32, 64)

		for j := 0; j < 16; j++ {
			w[j] = cast.BitsToUint32(block[j*32 : (j+1)*32])
		}
		...
		for j := 0; j < 64; j++ {
			t1 := h8 + bigSigma1(e) + ch(e, f, g) + k[j] + w[j]
